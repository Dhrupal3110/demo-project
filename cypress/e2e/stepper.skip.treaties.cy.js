describe('Stepper skip treaties flow', () => {
  const next = () => cy.get('#stepper-next').click();
  const previous = () => cy.get('#stepper-prev').click();

  const selectFirstDatabase = () => {
    cy.contains('h2', 'Select databases', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
    cy.get('table tbody tr', { timeout: 10000 })
      .first()
      .within(() => {
        cy.get('input[type="checkbox"]').should('not.be.checked').check({ force: true });
        cy.get('input[type="checkbox"]').should('be.checked');
      });
  };

  const selectPortfolio = () => {
    cy.contains('h2', 'Select portfolios', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
    cy.contains('button', 'EDM_RH_39823_AutoOwners_EQ_19', { timeout: 10000 }).click();
    cy.get('table tbody tr', { timeout: 10000 })
      .first()
      .within(() => {
        cy.get('input[type="checkbox"]').check({ force: true }).should('be.checked');
      });
  };

  const configureDemandSurge = () => {
    cy.contains('h2', 'Set demand surge', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
    cy.get('table tbody tr', { timeout: 10000 })
      .first()
      .within(() => {
        cy.get('input[type="checkbox"]').check({ force: true });
        cy.get('input[type="text"]').clear().type('Automation justification');
        cy.get('input[type="text"]').should('have.value', 'Automation justification');
      });
  };

  const configurePortfolioPerils = () => {
    cy.contains('h2', 'portfolio peril coverage', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
    cy.get('table tbody tr', { timeout: 10000 })
      .first()
      .within(() => {
        cy.get('td').eq(4).as('wsCell');
      });
    cy.get('@wsCell').click().click();
    cy.get('@wsCell').should('have.class', 'bg-(--color-primary-dark)');
  };

  const configurePortfolioRegions = () => {
    cy.contains('h2', 'portfolio region coverage', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
    cy.get('.grid.grid-cols-2', { timeout: 10000 })
      .first()
      .within(() => {
        cy.get('input[type="checkbox"]').first().check({ force: true }).should('be.checked');
      });
    cy.contains('h3', 'Regional Coverage')
      .parent()
      .parent()
      .find('input[type="checkbox"]')
      .first()
      .check({ force: true });
    cy.contains('button', 'Add').click();
  };

  const advanceToTreatiesStep = () => {
    selectFirstDatabase();
    next();

    selectPortfolio();
    next();

    configureDemandSurge();
    next();

    configurePortfolioPerils();
    next();

    configurePortfolioRegions();
    next();

    cy.contains('h2', 'Select treaties', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
  };

  it('skips treaty configuration steps when no treaties selected', () => {
    cy.visit('/stepper?id=PRG001');

    advanceToTreatiesStep();

    // Do not select any treaties, simply proceed
    next();

    cy.contains('h1', 'Review analyses', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');

    // The skipped steps should not render
    cy.contains('h2', 'treaty peril coverage', { matchCase: false }).should('not.exist');
    cy.contains('h2', 'treaty region coverage', { matchCase: false }).should('not.exist');
    cy.contains('h2', 'Link portfolios and treaties', { matchCase: false }).should('not.exist');

    // Navigate back and ensure we jump directly to the treaties step
    previous();

    cy.contains('h2', 'Select treaties', {
      matchCase: false,
      timeout: 10000,
    }).should('be.visible');
  });
});

